...
class ExactInference(InferenceModule):
	...
	#print 'pacmanPosition = ', pacmanPosition
        ...
        ############################ B(Xt+1) = P(Et+1|Xt+1) * B'(Xt+1)
        if noisyDistance == None:
        	allPossible = util.Counter() #INITIALIZE THE PROBABILITIES
        	allPossible[self.getJailPosition()] = 1.0  #->CATCH THE GHOST, GHOST IN THE JAIL -> P=1
	...
	def getDistribution( self, state ):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        ############################ B'(Xt+1) = SIGMA P(X'|Xt) * B(Xt)
        allPossible = util.Counter()
        for oldPos in self.legalPositions:
            	newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            	#print "newPosDist=", newPosDist
            	for newPos, prob in newPosDist.items():
            		#print "newPos, pro=", newPos,",", prob
            		allPossible[newPos] = allPossible[newPos] + prob * self.beliefs[oldPos]
            		#print "allPossible[newPos]=", allPossible[newPos]
            	#print "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
        #print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
	self.beliefs = allPossible        
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #print "initializeUniformly"
        self.particle = [0 for x in range(self.numParticles)]
        particlecount = 0

        while particlecount < self.numParticles:
		for p in self.legalPositions:
			if particlecount < self.numParticles:
				self.particle[particlecount] = p
    ...
    def observe(self, observation, gameState):
        ...
        #print "observe"
        ############################
        if noisyDistance == None:  ## all particles in jail
        	for count in range(self.numParticles):
        		self.particle[count] = self.getJailPosition()
        else:
        	newbelief = util.Counter()
        	beliefdist = self.getBeliefDistribution()
		for p in self.legalPositions:
		    	trueDistance = util.manhattanDistance(p, pacmanPosition)
		    	newbelief[p] = emissionModel[trueDistance] * beliefdist[p]
		
		if not any(newbelief.values()):
			self.initializeUniformly(gameState)
		else:
			newparticle = []
			for count in range(self.numParticles):
                    		newparticle.append(util.sample(newbelief)) 
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        belief = util.Counter()  ## initialize 0
        
        for position in self.particle:
            belief[position] = belief[position] + 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # cartesianProductList = list(cartesianProduct)
        # random.shuffle(cartesianProductList)
        # for i in range(self.numParticles):
        #     self.particles.append(tuple(i % len(cartesianProductList)))

        cartesianProduct = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(cartesianProduct)
        self.particles = [tuple(cartesianProduct[k % len(cartesianProduct)]) for k in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        for ghost in range(self.numGhosts):
            if noisyDistances[ghost] == None:
                ...
                for particle in self.particles:
                    newParticles.append(self.getParticleWithGhostInJail(particle, ghost))
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] != None:
                    ...
                    # trueDistance = util.manhattanDistance(pacmanPosition, ghost)
                    trueDistance = util.manhattanDistance(pacmanPosition, particle[ghost])
                    weights[particle] = weights[particle] * emissionModels[ghost][trueDistance]
        ...
        if weights.totalCount() == 0:
            ...
            for particle in self.particles:
                beliefDistribution[particle] *= weights[particle]
                weights[particle] = 1.0
            beliefDistribution.normalize()
            for particle in range(self.numParticles):
                self.particles[particle] = util.sample(beliefDistribution)
